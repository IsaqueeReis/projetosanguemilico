...
export const EssayPanel = ({ user, hasPremium = false }: Props) => {
  ...
  const handleDeleteTopic = async (id: string) => {
      if(!window.confirm("ATENÇÃO: Tem certeza que deseja excluir este tema? Isso pode afetar redações vinculadas.")) return;
      try {
  ...
  const handleDeleteSubmission = async (e: React.MouseEvent, id: string) => {
      e.stopPropagation();
      if(!window.confirm("Confirmar exclusão desta redação? Esta ação é irreversível.")) return;
      try {
          await EssayService.deleteSubmission(id);
          alert("Redação removida do sistema.");
          loadData();
          if (adminReviewSub?.id === id) setAdminReviewSub(null);
      } catch (e) {
          console.error(e);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
                    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 h-fit">
                        <h3 className="text-white font-bold mb-4 uppercase flex items-center gap-2">
                            ...
                            {adminTab === 'CORRECTIONS' ? 'Envios Pendentes' : 'Redações Recebidas'}
                        </h3>
                        <div className="space-y-3 max-h-[600px] overflow-y-auto custom-scrollbar pr-2">
                                ...
                                <div key={sub.id} onClick={() => handleAdminOpenCorrection(sub)} className={`p-4 bg-zinc-950 border rounded-lg cursor-pointer transition group ${adminTab === 'HISTORY' ? 'border-zinc-800 hover:border-green-600' : 'border-zinc-800 hover:border-red-600'}`}>
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button onClick={(e) => handleDeleteSubmission(e, sub.id)} className="p-1.5 text-zinc-600 hover:text-red-500 hover:bg-red-900/20 rounded transition" title="Excluir Redação"><Trash2 size={16}/></button>
                                        </div>
                                    </div>
                            ...
                            {submissions.filter(s => adminTab === 'CORRECTIONS' ? s.status === 'PENDING' : s.status === 'DONE').length === 0 && (
                                <p className="text-zinc-500 italic p-4 text-center border border-zinc-800 rounded-lg border-dashed">
                                    {adminTab === 'CORRECTIONS' ? 'Nenhum envio pendente.' : 'Nenhuma redação corrigida.'}
                                </p>
                            )}
                        </div>
                    </div>
                            ...
                            {topics.map(topic => (
                                <div key={topic.id} className="bg-zinc-950 border border-zinc-800 p-5 rounded-lg flex flex-col gap-3 group hover:border-zinc-700 transition">
                                    ...
                                    <div className="pt-3 border-t border-zinc-900 flex justify-between items-center">
                                        ...
                                        <button onClick={() => handleDeleteTopic(topic.id)} className="p-2 text-zinc-500 hover:text-red-500 hover:bg-red-900/20 rounded transition" title="Excluir Tema"><Trash2 size={18}/></button>
                                    </div>
                                </div>
                            ))}
                            {topics.length === 0 && (
