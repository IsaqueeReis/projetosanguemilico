...
import { Brain, Calendar, Check, Save, Sparkles, Trash2, List, FileText, ChevronRight, Activity, Zap, Scale, BarChart2, Plus, X, AlertTriangle, Key } from 'lucide-react';

// Função auxiliar para recuperar a chave (Ambiente ou Local)
const getApiKey = () => {
...
export const AdminMentorshipPanel = ({ users, plans }: AdminMentorshipPanelProps) => {
  ...
  const handleSaveApiKey = () => {
      if (localKey.startsWith('AIza')) {
          ...
          alert('Chave API salva no navegador! O sistema de IA está pronto.');
          setShowKeyInput(false);
      } else {
          alert('Chave API inválida. Deve começar com "AIza".');
      }
  };
  ...
  const handleAnalyzeEdital = async () => {
      ...
      if (!apiKey) {
          ...
          return alert("⚠️ ATENÇÃO: Chave API não detectada.\n\nPor favor, insira sua chave do Google Gemini no campo que apareceu no topo para ativar a IA.");
      }
      ...
      try {
          ...
          console.error(err);
          alert("Falha na Inteligência: Não foi possível estruturar o edital. Verifique sua chave API ou tente limpar o texto.");
      } finally {
  ...
  const handleGenerateSchedule = async () => {
      ...
      if (!apiKey) {
          setShowKeyInput(true);
          return alert("⚠️ ATENÇÃO: Chave API ausente. Insira no campo acima.");
      }
    ...
    <div className="bg-[#09090b] text-zinc-200 p-6 min-h-screen font-sans border border-zinc-900 rounded-xl">
      ...
      {!loading && currentPlan && activeMode === 'PLANNER' && (
          <div className="animate-fade-in">
              ...
              {/* CONFIGURAÇÃO MANUAL DE API KEY (Se não injetada pelo Netlify) */}
              {(showKeyInput || !getApiKey()) && (
                  <div className="bg-yellow-900/10 border border-yellow-700/50 p-4 rounded-xl mb-6 flex flex-col md:flex-row gap-4 items-center justify-between">
                      <div className="flex items-center gap-3">
                          <AlertTriangle className="text-yellow-500" size={24} />
                          <div>
                              <h4 className="text-yellow-500 font-bold text-sm uppercase">Configuração de Inteligência (Fallback)</h4>
                              <p className="text-zinc-400 text-xs">O servidor não detectou a chave de API. Insira manualmente para ativar o Gerador.</p>
                          </div>
                      </div>
                      <div className="flex gap-2 w-full md:w-auto">
                          <div className="relative flex-1 md:w-64">
                              ...
                              <input 
                                  ...
                                  className="w-full bg-zinc-950 border border-zinc-800 rounded pl-9 pr-3 py-2 text-sm text-white outline-none focus:border-yellow-500"
                              />
                          </div>
                          <button onClick={handleSaveApiKey} className="bg-yellow-600 hover:bg-yellow-700 text-black font-bold px-4 py-2 rounded text-xs uppercase">Salvar</button>
                      </div>
                  </div>
              )}
